science_pairs = [
    [
        r"what are the laws of thermodynamics",
        ["I'm not a physicist, but I think this has something to do with heat, entropy, and conservation of energy, right?",]
    ],
    [
        r"what disease does a carcinogen cause",
        ["Cancer.",]
    ],
    [
        r"what is a wavelength",
        ["Wavelength is the inverse of frequency.",]
    ],
    [
        r"what is thermodynamics",
        ["The branch of physics dealing with the transformation of heat to and from other forms of energy, and with the laws governing such conversions of energy.",]
    ],
    [
        r"what is chemistry",
        ["The science of mixing chemicals.",]
    ],
    [
        r"what is crystallography",
        ["This is the science dealing with the study of crystals.",]
    ],
    [
        r"what is avogadro's number",
        ["It is the number of molecules per mole. The numerical value is six point zero two times ten to the twenty third power.",]
    ],
    [
        r"what is ultrasound",
        ["Ultrasonic waves, used in medical diagnosis and therapy, in surgery, etc.",]
    ],
    [
        r"what is bioinformatics",
        ["A fancy name for applied computer science in biology.",]
    ],
    [
        r"what is ichthyology",
        ["We talk about this when we study fishes.",]
    ],
    [
        r"what is h2o",
        ["H is to O as O is to V.",]
    ],
    [
        r"what is cytology",
        ["The study of cells.", "Well, from what I can recall, it is the study of cells.",]
    ],
    [
        r"what is wavelength",
        ["In physics, the distance, measured in the direction of progression of a wave, from any given point to the next point characterized by the same phase. Or it could be looked at as a way of thinking.",]
    ],
    [
        r"what is bacteriology",
        ["This is the scientific study of bacteria and diseases caused by them.",]
    ],
    [
        r"what is gravitation",
        ["An invitation to a burial.", "The force by which every mass or particle of matter, including photons, attracts and is attracted by every other mass or particle of matter.",]
    ],
    [
        r"we are on the same wavelength",
        ["And the same frequency.",]
    ],
    [
        r"how far is the sun",
        ["The sun is about 93 million miles from Earth.",]
    ],
    [
        r"how far is the moon",
        ["The moon is about 250,000 miles from Earth on average.",]
    ],
    [
        r"do you know chemistry",
        ["What is chemistry?",]
    ],
    [
        r"do you understand thermodynamics",
        ["What is thermodynamics?",]
    ],
    [
        r"chemistry",
        ["My favorite subject is chemistry.",]
    ],
    [
        r"tell me about venus",
        ["Venus is the second planet from the sun.",]
    ],
]
